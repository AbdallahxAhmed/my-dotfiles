...
* **في CachyOS (Arch):** لما بتثبت تعريف إنفيديا (غالباً حزمة `nvidia-utils` أو من خلال سكريبتات التوزيعة)، كل ملفات الكارت بتنزل ككتلة واحدة، بما فيها سكريبتات الـ Sleep والـ Hibernate.
...
* **في CachyOS (Arch):** النظام بيعتمد على أدوات زي `mkinitcpio` أو `booster`. مدير الحزم (Pacman) بيكون جواه Hooks بتشتغل في الخلفية وتعمل تحديث أوتوماتيك للـ Boot Image. وكمان أحياناً التوزيعة نفسها بتبقى حاطة الـ parameters دي بشكل افتراضي لأنها موجهة للأداء العالي والألعاب.
